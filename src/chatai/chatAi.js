...
export default function Chatai() {
  ...
  useEffect(() => {
    // Set a delay before showing the second message with typing animation
    const timer = setTimeout(() => {
      addTypingMessage(initialMessages[1]);
    }, 500); // 0.5-second delay before showing the next message
  ...
  const handleSend = async () => {
    if (input.trim() !== "" && turn <= 5) {
      ...
      if (turn === 5) {
        ...
        }, 3000); // 1-second delay before navigating to the loading page
      }
